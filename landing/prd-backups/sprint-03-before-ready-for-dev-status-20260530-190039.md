...
Avisar o comprador quando documento estiver pronto em seu nome e enviar o arquivo recebido do despachante por WhatsApp ou e-mail, conforme canal configurado.

Arquivo detalhado: `stories/S3-US08-documento-pronto-aviso-comprador.md`.

Escopo inicial:
- Integrar/registrar retorno do despachante por e-mail ou WhatsApp.
- Vincular arquivo recebido ao processo correto de venda/documentacao.
- Se o vinculo for seguro, enviar automaticamente ao comprador quando houver integracao aprovada; se nao houver provedor ativo, preparar envio assistido pelo Administrativo informando que o documento esta pronto em seu nome, com arquivo anexo quando permitido.
- Se o vinculo nao for seguro, exigir conciliacao manual do Administrativo antes de qualquer envio ao comprador.
- Registrar canal, data/hora, destinatario, arquivo, responsavel/automacao, status de envio, falhas e reenvios.
### S3-US09: Dossie da Venda, Dados e Metricas Gerais
Gravar todos os dados, documentos, eventos e conclusoes do processo de venda no banco e no dossie digital, para alimentar metricas gerais da loja e futuras analises com IA.

Arquivo detalhado: `stories/S3-US09-dossie-venda-dados-metricas.md`.

Escopo inicial:
- Organizar contratos, garantias, laudos, comprovantes, recibos, transferencia, documentos assinados, comunicacoes e evidencias no dossie da venda.
- Registrar tudo no banco com vinculos entre venda, veiculo, comprador, vendedor, despachante, arquivos e eventos.
- Alimentar dashboards gerais com prazos, pendencias, gargalos, receita adicional, custos, spread, status de documentacao e comunicacoes.
- Guardar historico de eventos com data/hora, responsavel, origem e antes/depois de status.
- Preparar dados estruturados para consumo futuro por IA e extracao de insights operacionais/comerciais.
...
- Automacao real de WhatsApp em producao sem provedor definido; enquanto isso, fluxo assistido/manual com auditoria. O detalhamento tecnico futuro podera considerar Evolution API, com instancia, autenticacao, anexos, webhooks, logs e tratamento de falhas.
- Assinatura digital integrada via API Gov.br se ainda nao houver integracao aprovada; usar fluxo manual assistido.

## Proximo Passo
Gerar sprint-03-ready-for-dev.md consolidando as 9 stories, dependencias, permissoes, criterios gerais de aceite e checklist tecnico para desenvolvimento.
