...
- Evitar decisoes irreversiveis quando uma solucao simples resolver o MVP.`r`n- Documentar alternativas e caminho de evolucao futura sempre que uma decisao tecnica puder mudar depois.


## Padrao Obrigatorio: Evolucao Futura
Toda decisao tecnica que possa mudar no futuro deve documentar:

- Decisao atual do MVP.
- Alternativas futuras possiveis.
- Quando considerar a mudanca.
- Como realizar a migracao em alto nivel.
- Quais dados, logs, permissoes, contratos e historicos precisam ser preservados.
- Riscos e cuidados da mudanca.

Esse padrao deve ser usado em infraestrutura, banco, storage, filas/jobs, integracoes, IA, OCR, canais, seguranca, permissoes, logs, deploy e qualquer outra esfera tecnica relevante.
...
## Decisao Aprovada: Monolito Modular
O MVP deve ser implementado como monolito modular.

- Microservicos ficam fora do inicio.
- O backend deve ser uma aplicacao principal organizada por dominios/modulos internos.
- Cada modulo deve ter responsabilidades claras, regras de negocio, servicos, validacoes e contratos internos.
- Integracoes externas devem ficar em adaptadores, evitando acoplamento direto no dominio.
- Jobs/workers podem rodar separados operacionalmente, mas mantendo o mesmo dominio e banco no MVP.

## Evolucao Futura: Extracao de Modulos
Se algum dominio crescer muito, ele podera ser extraido no futuro para servico separado.

Candidatos naturais de extracao futura:

- Automacoes/jobs.
- IA/OCR/leitura assistida.
- Integracoes de marketplaces.
- Mensageria WhatsApp/e-mail.
- Relatorios/dashboards pesados.

Considerar extracao apenas se houver:

- Alto volume de processamento.
- Gargalo de performance.
- Necessidade de escala independente.
- Necessidade de isolamento de seguranca.
- Equipe ou manutencao separada.
- Custo/risco operacional justificando a separacao.

Como migrar no futuro:

1. Manter contrato/API interno do modulo.
2. Separar worker ou servico daquele dominio.
3. Criar fila, banco ou storage proprio somente se necessario.
4. Substituir chamada interna por chamada HTTP, evento ou fila.
5. Preservar RBAC, auditoria, logs, historico, snapshots e correlacao com processos.
6. Migrar dados com plano de rollback e validacao.
7. Manter compatibilidade com o backend central enquanto a transicao acontece.
## Regras de Separacao
...
5. Aprovado: MVP sera monolito modular; microservicos ficam fora do inicio, com roteiro documentado para extracao futura de modulos se necessario.
