...
async function confirmSubscription(req, res, next) {
  try {
    ...
    if (status === 'active' || status === 'trialing') {
      // Idempotency: a retry of this exact submission (client timeout,
      // double-click) now reuses the same Stripe subscription (see the
      // idempotency key on subscriptions.create in stripeService.js), so
      // `subscriptionId` is the same value as the original attempt — if that
      // attempt already fully completed, the establishment already exists.
      // Without this check the retry would hit createFromStripe's cnpj/email
      // uniqueness guard and surface a confusing 409 for a signup that
      // actually already succeeded.
      const existing = await prisma.establishment.findFirst({ where: { stripeSubscriptionId: subscriptionId } });
      if (existing) {
      ...
      const created = await establishmentService.createFromStripe({
        nome:                nome.trim(),
        cnpj,
        telefone,
      ...
      // A genuinely concurrent double-submit can still race past the
      // `existing` check above — createFromStripe signals that case so we
      // don't email/return `password`, which was generated by this (losing)
      // request and does not match the winner's actual account password.
